...
/* ₱20 - Orange */
.btn-orange {
  background-color: #FF7300; /* vivid orange */
  color: #fff;
}
.btn-orange:hover {
...
/* ₱50 - Red */
.btn-red {
  background-color: #CE1127; /* deep red */
  color: #fff;
}
.btn-red:hover {
...
/* ₱200 - Green */
.btn-green {
  background-color: #2C9A4C; /* BSP green */
  color: #fff;
}
.btn-green:hover {
  background-color: #248a3f;
  color: #fff;
}

/* ₱500 - Yellow */
.btn-yellow {
  background-color: #F6C23E; /* warm yellow */
...
/* ₱1000 - Blue */
.btn-blue {
  background-color: #378ce7ff; /* bright BSP blue */
  color: #fff;
}
.btn-blue:hover {
        ...
        <div class="flex grid">
            ...
            <main x-data="productsApp()">
                <div x-data="cartHandler" class="main-container">
                    ...
                    <div 
    class="cart-section forms" 
    :class="{'visible': cartVisible}" 
    x-show="cartVisible" 
    x-cloak
>
<div class="cash-drawer d-flex justify-content-between align-items-center">
    <h5 class="mb-0">
        Register Balance: ₱<span x-text="formatCash(cashDrawer)"></span>
    </h5>
    <?php if ($userRole === 'ADMIN'): ?>
        <button type="button" class="btn btn-outline-success ms-3"
                data-bs-toggle="modal" data-bs-target="#cashDrawerModal">
            View Cash Register
        </button>
    <?php endif; ?>
</div>
                                        ...
                                        <form id="orderForm" action="api/cashier_controller.php" method="POST" @submit.prevent="proccessOrder">
                                            ...
                                            <template x-for="(cart, i) in carts" :key="cart.product.id + '-' + cart.price">
                                                <div>
                                                    ...
                                                    <span class="font-bold" x-text="discountAmount ? discountAmount + '%' : 'No Discount'"></span>
                                                                                                    ...
                                                                                                    <button type="button" class="btn btn-warning btn-sm" @click="applyDiscountModal()">Apply Discount</button>  
                                            ...
                                            <div class="input-group mb-4 d-flex">
                                                <div class="input-group-prepend">
                                                    <span class="input-group-text">Tendered Amount:</span>
                                                </div>
                                            ...
                                            <div>
                                                <div class="d-grid gap-2 mb-3" style="grid-template-columns: repeat(5, 1fr);">
                                                    <button  type="button" @click="addAmount(1)" class="btn btn-coin">₱1</button>
                                                    <button  type="button" @click="addAmount(5)" class="btn btn-coin">₱5</button>
                                                    <button  type="button" @click="addAmount(10)" class="btn btn-coin">₱10</button>
                                                    <button  type="button" @click="addAmount(20)" class="btn btn-orange">₱20</button>
                                                    <button  type="button" @click="addAmount(50)" class="btn btn-red">₱50</button>
                                                </div>
                                                
                                                <div class="d-grid gap-2" style="grid-template-columns: repeat(5, 1fr);">
                                                    <button type="button" @click="addAmount(100)" class="btn btn-purple">₱100</button>
                                                    <button type="button" @click="addAmount(200)" class="btn btn-green">₱200</button>
                                                    <button type="button" @click="addAmount(500)" class="btn btn-yellow">₱500</button>
                                                    <button type="button" @click="addAmount(1000)" class="btn btn-blue">₱1000</button>
                                                    <button type="button" @click="clearAmount()" class="btn btn-danger">Clear</button>
                                                </div>
                                            </div>
...
<script>
    ...
    return {
        ...
        initialCash: 0,
        cashDrawer: 0,
        change: 0,  // Ensure cashDrawer is properly initialized
...
range(start, end, step = 1) {
  ...
  if (this.payment >= this.totalPrice) {
    ...
    try {
      ...
      // Calculate change and append it to the form data
      const change = this.payment - this.totalPrice;
